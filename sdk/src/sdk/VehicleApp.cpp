...
VehicleApp::VehicleApp(std::shared_ptr<IVehicleDataBrokerClient> vdbClient,
    ...
    : m_vdbClient(vdbClient), m_pubSubClient(std::move(pubSubClient)) {
...
AsyncSubscriptionPtr_t<std::string> VehicleApp::subscribeToTopic(
    const std::string& topic) {
  if (m_pubSubClient) {
    return m_pubSubClient->subscribeTopic(topic);
  }
  logger().error(
      "subscribeToTopic(...) disfunctional: App has no PubSubClient "
      "instantiated");
  return {};
}
...
AsyncResultPtr_t<DataPointReply> VehicleApp::getDataPoints(
...
AsyncResultPtr_t<DataPointReply> VehicleApp::getDataPoint_internal(
...
AsyncSubscriptionPtr_t<DataPointReply> VehicleApp::subscribeDataPoints(
    const std::string& query) {
  return m_vdbClient->subscribe(query);
}

void VehicleApp::publishToTopic(const std::string& topic,
                                const std::string& data) {
  if (m_pubSubClient) {
    m_pubSubClient->publishOnTopic(topic, data);
  } else {
    logger().error(
        "publishToTopic(...) ignored: App has no PubSubClient instantiated");
  }
}

PublishStatus VehicleApp::publishOnTopic(const std::string& topic,
                                         const std::string& data,
                                         int timeout_ms) {
  return m_pubSubClient->publishOnTopic(topic, data, timeout_ms);
}

void VehicleApp::reconnect(int timeout_ms) {
  m_pubSubClient->reconnect(timeout_ms);
}

std::shared_ptr<IVehicleDataBrokerClient>
VehicleApp::getVehicleDataBrokerClient() {
  return m_vdbClient;
}

std::shared_ptr<IPubSubClient> VehicleApp::getPubSubClient() {
