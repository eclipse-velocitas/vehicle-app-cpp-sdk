...
TEST(Test_Job, callCtor_none_noFailure) {
    EXPECT_NO_THROW(Job([] {}));
}

TEST(Test_Job, create_firstCall_noFailureAndNoNullPtr) {
    JobPtr_t job;
    EXPECT_NO_THROW(job = Job::create([] {}));
    EXPECT_TRUE(job);
}

TEST(Test_Job, create_furtherCall_noFailureAndReturnsNewValidPtr) {
...
TEST(Test_Job, shallRecur_none_returnsFalse) {
    Job job([] {});
    EXPECT_FALSE(job.shallRecur());
}

TEST(Test_Job, createJob_none_passedFunctionIsNotExecuted) {
    bool executeWasCalled = false;
    Job  job([&executeWasCalled] { executeWasCalled = true; });
    EXPECT_FALSE(executeWasCalled);
}

TEST(Test_Job, execute_none_passedFunctionIsExecuted) {
    bool executeWasCalled = false;
    Job  job([&executeWasCalled] { executeWasCalled = true; });
...
TEST(Test_Job, getFunction_notExecuted_returnedFunctionCanBeExecuted) {
    bool executeWasCalled = false;
    Job  job([&executeWasCalled] { executeWasCalled = true; });
...
TEST(Test_Job, waitForTermination_notExecuting_ImmediatelyReturns) {
    bool executeWasCalled = false;
    Job  job([&executeWasCalled] { executeWasCalled = true; });
...
TEST(Test_Job, waitForTermination_executing_doesNotReturnWhileExecuting) {
    ...
    Job                     job([&] {
        ...
        }
        {
            ...
            executorCV.wait(lock, [&] { return finishExecution; });
        }
    });
    ...
    // make sure scavenger is waiting
    {
        ...
        scavengerCV.wait(lock, [&] { return startedWaiting; });
        std::this_thread::sleep_for(2ms);
    }

    EXPECT_FALSE(finishedWaiting);
...
TEST(Test_Job, finishExecution_waitingForTermination_returnsImmediatly) {
    ...
    {
        std::unique_lock lock{scavengerMutex};
        scavengerCV.wait(lock, [&] { return finishedWaiting; });
        EXPECT_TRUE(finishedWaiting);
    }
...
TEST(Test_RecurringJob, callCtor_none_noFailure) {
    EXPECT_NO_THROW(RecurringJob([] {}));
}

TEST(Test_RecurringJob, create_firstCall_noFailureAndNoNullPtr) {
    JobPtr_t job;
    EXPECT_NO_THROW(job = RecurringJob::create([] {}));
    EXPECT_TRUE(job);
}

TEST(Test_RecurringJob, create_jobsCreatedBefore_noFailureAndReturnsNewValidPtr) {
...
TEST(Test_RecurringJob, cancel_none_noFailure) {
    RecurringJob job([] {});
...
TEST(Test_RecurringJob, createRecurringJob_none_passedFunctionIsNotExecuted) {
    bool         executeWasCalled = false;
