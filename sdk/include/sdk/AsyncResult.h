...
class AsyncResult {
 public:
  ...
  TResultType await() {
    if (m_callback) {
      throw std::runtime_error(
          "Invalid usage: Either call await() or register an onResult "
          "callback!");
    }
  ...
  /**
   * @brief Calls the specified callback when an error occurrs during async
   * execution. The callback invocation is done by a worker thread.
   *
   * @param callback      The callback to invoke.
   * @return AsyncResult* This for method chaining.
   */
  ...
  /**
   * @brief Map the result to a different type using the provided mapper
   * function.
   *
   ...
   *                     A new AsyncResult object which emits results of
   * TNewType.
   */
  template <typename TNewType>
  std::shared_ptr<AsyncResult<TNewType>> map(
      std::function<TNewType(const TResultType&)> mapper) {
    auto mappedResult = std::make_shared<AsyncResult<TNewType>>();

    onResult([mappedResult, mapper](auto item) {
      mappedResult->insertResult(mapper(item));
    });

    onError([mappedResult](auto status) {
      mappedResult->insertError(std::move(status));
    });
...
template <typename T>
using AsyncResultPtr_t = std::shared_ptr<AsyncResult<T>>;
...
template <typename T>
using AsyncSubscriptionPtr_t = std::shared_ptr<AsyncSubscription<T>>;
